...
function AddDefaultBannerNewArrival() {
  const [image, setImage] = useState(null);
  const [location, setLocation] = useState('New Arrival');
  ...
  const handleSubmit = async (e) => {
    ...
    if (image && location) {
      try {
        ...
        const response = await fetch('http://localhost/waltzify_copy_fake/Backend/Add_default_banner_new.php', {
          method: 'POST',
          body: formData,
        });
  ...
  return (
    <div className='bg-[#F2F6F9] py-[2rem]'>
      <div className='flex flex-col lg:flex-row justify-between lg:items-center px-[2rem] lg:px-[4rem]'>
        <p className='text-xl lg:text-3xl font-bold'>Add Default Banner(New Arrival Page)</p>
      ...
      <div className='flex flex-col gap-[3rem] bg-white rounded-xl my-[2rem] mx-[2rem] lg:mx-[4rem] py-[2rem]'>
        ...
        <div className='px-[2rem] flex flex-col lg:flex-row lg:items-center justify-between'>
          <p className='font-bold'>
            Upload Image (Dimensions range 410 x 123 and above is required) <span className='text-red-500'>*</span>
          </p>
...
export default AddDefaultBannerNewArrival;
