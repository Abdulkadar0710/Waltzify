...
const Reconcilliation = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await fetch('http://localhost/waltzify_copy_fake/Backend/fetch_reconcilliation.php');
        if (!response.ok) {
    ...
    // Export orders to Excel
      ...
      const exportToExcel = () => {
        // Extract only the orders that are visible on the current page
        const visibleOrders = Object.keys(groupedOrders)
          .filter((orderId) => {
            const order = groupedOrders[orderId][0];
      
            // Apply the same filtering logic you use for display
            if (order.paymode === 'Prepaid' || order.trackingId === '' || order.courier_company === '') {
              return false;
            }
          ...
          .slice(indexOfFirstOrder, indexOfLastOrder) // Paginate orders
          .flatMap(orderId => groupedOrders[orderId]); // Flatten the grouped orders array into a single list of products.
      
        // Use a Set to avoid duplicate entries
        const uniqueOrders = Array.from(new Set(visibleOrders.map(order => order.OrderId)))
          .map(uniqueId => visibleOrders.find(order => order.OrderId === uniqueId));
      
        // Prepare the data in the required format for Excel
        const ordersForExcel = uniqueOrders.map(order => ({
          ...
          'Delivery Address': `${order.Address1}, ${order.City}`,
          'State':order.State,
          'Tracking ID': order.trackingId || 'N/A', // If tracking ID is missing, use 'N/A'
          'Courier Company': order.courier_company || 'N/A' // If courier company is missing, use 'N/A'
        }));
      
        if (ordersForExcel.length > 0) {
          const worksheet = XLSX.utils.json_to_sheet(ordersForExcel); // Convert the visible orders to a worksheet
          ...
          a.href = url;
          a.download = 'reconcilliation.xlsx'; // File name for the download
          a.click();
          window.URL.revokeObjectURL(url); // Clean up the URL
        } else {
          alert('No visible orders available to export!');
        }
      };
      ...
      if (data.success) {
        ...
        const updatedProducts = products.map((product) => {
          if (product.OrderId === OrderId) {
            ...
            return { ...product, payment_status: 'Success' };
          }
          return product;
        });
...
const handleClickNo = (OrderId) => {
    ...
    .then(data => {
      if (data.success) {
        alert('Payment status updated successfully');
         /* setProducts(prevOrders => prevOrders.map(product => 
          product.OrderId === OrderId ? { ...product, payment_status: 'COD Payment Not Received' } : product
        ));  */
        const updatedProducts = products.map((product) => {
          if (product.OrderId === OrderId) {
            // Update the product's trackingIdmail to 'Mailed'
            return { ...product, payment_status: 'COD Payment Not Received' };
          }
          return product;
        });
      ...
      <table className="w-full table-auto border-collapse bg-violet-20">
        <thead>
            <tr className="bg-violet-200">
                ...
                <th className="border p-2">Payment Recieved(COD)</th>
                                ...
                                <>
                                    ...
                                    <td className="border p-2" rowSpan={groupedOrders[orderId].length}>
                                        ...
                                        <span className={`font-bold ${
                                            ...
                                            product.payment_status === 'COD Payment Not Received' ? 'text-red-500' : 
