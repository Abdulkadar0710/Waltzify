...
const handleShipping = () => {
  ...
  .then(data => {
      if (data.success) {
          alert('Shipping Status is updated successfully.');
...
const exportSelectedOrders = () => {
    ...
    const selectedOrdersData = selectedOrders.map(orderId => {
        ...
        return {
            "Order Details": `${firstOrder.OrderId}`,
            ...
            "Pincode":firstOrder.Pincode,
            "Delivery Address": firstOrder.Address1,
            "State" : firstOrder.State,
            "Product Details": orderGroup.map(product => 
                `${product.orderItemProductName}, SKU: ${product.SKU}, QTY: ${product.totalQuantityOrdered}`
