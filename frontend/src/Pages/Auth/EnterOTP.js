...
function EnterOTP() {
    ...
    const verifyOtp = (e) => {
        e.preventDefault();
        const url = 'http://localhost/waltzify_copy_fake/Backend/verify_otp.php';
        const headers = {
        ...
        fetch(url, {
            ...
            .then((response) => {
                if (response.result === 'Invalid OTP') {
                    ...
                    setTimeout(() => {
                        navigate('/resetpassword'); // Navigate to password reset page
                    }, 1000);
                }
            })
            .catch((err) => {
    ...
    return (
        <div className='flex mt-[12vw] lg:mt-[10vw] items-center justify-center'>
            <form className='mt-[5rem] flex flex-col gap-4' onSubmit={verifyOtp}>
                ...
                <button className='ml-[4rem] bg-orange-500 text-white mb-[10rem] rounded-xl h-[3rem] w-[10rem]' type='submit' disabled={timeRemaining <= 0}>Verify OTP</button>
            </form>
        </div>
    );
}
